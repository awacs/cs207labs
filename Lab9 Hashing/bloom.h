...
index_t hash1(bloom_filter_t *B, ourkey_t k);//done
index_t hash2(bloom_filter_t *B, ourkey_t k);//done

void bloom_init(bloom_filter_t *B, index_t size_in_bits);//done
void bloom_destroy(bloom_filter_t *B);//done
int bloom_check(bloom_filter_t *B, ourkey_t k);//done
void bloom_add(bloom_filter_t *B, ourkey_t k);//done
